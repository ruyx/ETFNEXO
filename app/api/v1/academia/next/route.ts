...
export async function GET(request: NextRequest) {
  ...
  if (!currentId) {
    console.error('[Academia Next API] Missing currentId parameter');
  ...
  try {
    ...
    console.log('[Academia Next API] Fetching next article after:', currentId);
    ...
    if (allError || !allArticles || allArticles.length === 0) {
      console.log('[Academia Next API] No published articles found:', allError?.message);
    ...
    if (currentIndex === -1) {
      console.log('[Academia Next API] Current article not found in list');
      return NextResponse.json(null, { status: 404 });
    }

    // Check if there's a next article (index + 1)
    if (currentIndex + 1 >= allArticles.length) {
      console.log('[Academia Next API] No more articles available - reached end of list');
      return NextResponse.json(null, { status: 404 });
    }
    ...
    if (nextError || !nextArticle) {
      console.log('[Academia Next API] Error fetching next article:', nextError?.message);
      return NextResponse.json(null, { status: 404 });
    }

    console.log('[Academia Next API] Found next article:', nextArticle.id, nextArticle.title);
    return NextResponse.json(nextArticle);
  } catch (error) {
    console.error('[Academia Next API] Error:', error);
